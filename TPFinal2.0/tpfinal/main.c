...
void consultaPalabra(nodoA* Arbol)
{
    ...
    /**PUNTO 2**/
int idEncontrado(nodoT*,int);
int idEncontrado(nodoT*lista,int id){
    while(lista){
        if(lista->idDOC==id){
            return 1;
        }
...
void mostrarIdPos(nodoT*,int);
void mostrarIdPos(nodoT*lista,int id){
    while(lista){
        if(lista->idDOC==id){
            printf("ID: %i, POS: %i\n",id,lista->pos);
        }
...
void comparacionOcurrencias(nodoA* arbol,char palabra[],int ids[],int valIds)
{
    nodoA* encontrado = existeNodo(arbol,palabra);
    int flag = 1,i=0;
    if(encontrado){
        nodoT*lista=encontrado->ocurrencias;
        while(i<valIds && flag == 1){
            flag=idEncontrado(lista,ids[i]);
            i++;
        }
        if(flag == 1){
            printf("LA PALABRA SE HA ENCONTRADO EN TODOS LOS DOCUMENTOS, EN LAS SIGUIENTES POSICIONES:\n");
            for(int j=0;j<valIds-1;j++){
                mostrarIdPos(lista,ids[j]);
            }
        }else{
            printf("ERROR: LA PALABRA NO SE ENCONTRO EN TODOS LOS DOCUMENTOS INDICADOS\n");
        }
    }
}
...
void interseccionArchivos(nodoA* arbol)
{
    FILE* fp = fopen("registroArchivos.bin","rb");
    int tam,valArch=0;
    char palabra[TAM_MAX],continuar;
    int archivos[100];
    if(fp)
    {
        ...
        if(tam>0){
            ...
            if(existeNodo(arbol,palabra)){
                do{
                    ...
                    scanf("%i",&archivos[valArch]);
                    printf("INGRESE 'S' PARA BUSCAR EN OTRO ARCHIVO\n");
                    fflush(stdin);
                    scanf("%c",&continuar);
                    valArch++;
                }while(valArch<tam+1 && (continuar=='s' || continuar=='S'));
                comparacionOcurrencias(arbol,palabra,archivos,valArch+1);
            }else{
